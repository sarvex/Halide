...
def _is_valid_name(name: str) -> bool:
    ...
    return False if name == "generator_params" else _NAME_RE.search(name)
...
class Generator(ABC):
    ...
    def call(cls, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            _check(
                k in input_names,
                f"Unknown input '{k}' specified via keyword argument.",
            )
...
def _fqname(o):
    ...
    m = k.__module__
    q = k.__qualname__
    return q if m == "__main__" or k == "builtins" else f"{m}.{q}"
...
def _check_generator_name_in_use(n:str):
    ...
    _check(
        n not in _python_generators,
        f"The Generator name {n} is already in use.",
    )
